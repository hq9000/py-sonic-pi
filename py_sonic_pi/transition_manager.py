...
class TransitionManager:
    ...
    def do_transition(self, new_project: Project, transition_time_bars: float) -> None:
        old_project = self._load_project_from_serialized_file(
            _SERIALIZED_PLAYING_PROJECT_FILE_NAME
        )
        self._save_serialized_project_to_file(
            new_project, _SERIALIZED_PLAYING_PROJECT_FILE_NAME
        )
        if not old_project:
        ...
        new_project.save_references = True
        self._render_projects_to_rb_file(
            [old_project, new_project], _OUTPUT_RB_FILE_NAME
        )

    def _render_projects_to_rb_file(
        self, projects: list[Project], file_path: str
    ) -> None:
    ...
    def _save_serialized_project_to_file(
        self, project: Project, file_path: str
    ) -> None:
